...
class PygAnimation(object):
        ...
        if frames != '_copy':  # ('_copy' is passed for frames by the getCopies() method)
            ...
            for i in range(self.numFrames):
                ...
                assert type(frame[0]) in (str, pygame.Surface), \
                    'Frame %s image must be a string filename or a pygame.Surface' \
                    % i
                assert frame[1] > 0, \
    ...
    def makeTransformsPermanent(self):
        self._images = [pygame.Surface(surfObj.get_size(), 0, surfObj)
    ...
    def play(self, startTime=None):
        ...
        if self._state == PLAYING:
            ...
            self._playingStartTime = startTime - (self._pausedStartTime
    ...
    def anchor(self, anchorPoint=NORTHWEST):
        ...
        for i in range(len(self._images)):
            ...
            if anchorPoint == NORTHWEST:
                ...
                newSurf.blit(self._images[i], (halfMaxWidth
                             - halfFrameWidth, 0))
            elif anchorPoint == NORTHEAST:
                newSurf.blit(self._images[i], (maxWidth - frameWidth,
                             0))
            elif anchorPoint == WEST:
                newSurf.blit(self._images[i], (0, halfMaxHeight
                             - halfFrameHeight))
            elif anchorPoint == CENTER:
                newSurf.blit(self._images[i], (halfMaxWidth
                             - halfFrameWidth, halfMaxHeight
                             - halfFrameHeight))
            elif anchorPoint == EAST:
                newSurf.blit(self._images[i], (maxWidth - frameWidth,
                             halfMaxHeight - halfFrameHeight))
            elif anchorPoint == SOUTHWEST:
                newSurf.blit(self._images[i], (0, maxHeight
                             - frameHeight))
            elif anchorPoint == SOUTH:
                newSurf.blit(self._images[i], (halfMaxWidth
                             - halfFrameWidth, maxHeight - frameHeight))
    ...
    def _makeTransformedSurfacesIfNeeded(self):
        ...
        if self._transformedImages == []:
            self._transformedImages = [surf.copy() for surf in
                    self._images]
    ...
    def flip(self, xbool, ybool):
        ...
        for i in range(len(self._images)):
            self._transformedImages[i] = \
                pygame.transform.flip(self.getFrame(i), xbool, ybool)
    ...
    def scale(self, width_height):
        ...
        for i in range(len(self._images)):
            self._transformedImages[i] = \
                pygame.transform.scale(self.getFrame(i), width_height)
    ...
    def rotate(self, angle):
        ...
        for i in range(len(self._images)):
            self._transformedImages[i] = \
                pygame.transform.rotate(self.getFrame(i), angle)
    ...
    def rotozoom(self, angle, scale):
        ...
        for i in range(len(self._images)):
            self._transformedImages[i] = \
                pygame.transform.rotozoom(self.getFrame(i), angle,
                    scale)
    ...
    def scale2x(self):
        ...
        for i in range(len(self._images)):
            self._transformedImages[i] = \
                pygame.transform.scale2x(self.getFrame(i))
    ...
    def smoothscale(self, width_height):
        ...
        for i in range(len(self._images)):
            self._transformedImages[i] = \
                pygame.transform.smoothscale(self.getFrame(i),
                    width_height)
    ...
    def _propSetElapsed(self, elapsed):
        ...
        if self._loop:
            ...
            elapsed = getInBetweenValue(0, elapsed,
                    self._startTimes[-1])
    ...
    def _propGetElapsed(self):
        ...
        if self._state == PLAYING:
            ...
            elapsed = (self._pausedStartTime - self._playingStartTime) \
                * self.rate
        if self._loop:
            elapsed = elapsed % self._startTimes[-1]
        else:
...
def findStartTime(startTimes, target):
    ...
    while True:
        ...
        if startTimes[i] == target or startTimes[i] < target \
            and startTimes[i + 1] > target:
            if i == len(startTimes):
                return i - 1
            else:
                return i
